...
void __always_inline submit_event(struct task_struct *task, u32 pid, u64 mntns, u32 syscall_id, unsigned char times)
{
    ...
    struct syscall_event *event =
        bpf_ringbuf_reserve(&event_map, sizeof(struct syscall_event), 0);
    if (!event)
    {
...
int sys_enter(struct trace_event_raw_sys_enter *args)
{
    ...
    if (bpf_map_lookup_elem(&command_map, &comm) == NULL)
    {
        bpf_map_update_elem(&command_map, &pid, &comm, BPF_ANY);
    }

    // Update the syscalls
    u8 *const syscall_value = bpf_map_lookup_elem(&syscall_map, &pid);
    if (syscall_value)
    {
    ...
    else
    {
        ...
        bpf_map_update_elem(&syscall_map, &pid, &init, BPF_ANY);

        u8 *const value = bpf_map_lookup_elem(&syscall_map, &pid);
        if (!value)
        {
